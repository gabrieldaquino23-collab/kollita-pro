/**
 * ============================================================
 * KOLLITA PRO — AUTH GLOBAL v2.0
 * Sistema de contraseñas unificado con sincronización Supabase
 * ============================================================
 */
...
function mostrarModalCambioPassGlobal(panelName, onSuccessCallback) {
    ...
    document.getElementById('ac-btn-save').onclick = async function() {
        ...
        // 2. Sincronizar con Supabase (async, no bloquea el login)
        status.textContent = '🔄 Sincronizando...';
        try {
            if (typeof SupabaseClient !== 'undefined') {
                await SupabaseClient.upsert('system_passwords', {
                    panel: panelName,
                    pass_hash: p1,  // En producción futura usar bcrypt desde el API
                    updated_by: 'user'
                }, 'panel');
                status.textContent = '☁️ Sincronizado con la nube';
            }
        } catch (syncErr) {
            // Si falla la sync, igual deja entrar (el sistema paracaídas tomará el control)
            status.textContent = '📴 Guardado local (sync pendiente)';
            if (typeof KollitaSync !== 'undefined') {
                KollitaSync.enqueue('system_passwords', 'UPSERT', {
                    panel: panelName,
                    pass_hash: p1,
                    updated_by: 'user'
                }, null);
